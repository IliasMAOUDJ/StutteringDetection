...
for filename in tqdm(glob.glob(os.path.join(train_path_stutter, '*.wav'))):
...
discarded = 0
for filename in tqdm(glob.glob(os.path.join(train_path_fluent, '*.wav'))):
...
print("Training data with S=",len(x_s),"and F=", len(x_f_h))
x_train = x_s + x_f_h
y_train = y_s + y_f_h
...
for filename in tqdm(glob.glob(os.path.join(test_path_stutter, '*.wav'))):
    stutter_np = conv_audio_data(filename)
...
discarded_t = 0
for filename in tqdm(glob.glob(os.path.join(test_path_fluent, '*.wav'))):
    fluent_np = conv_audio_data(filename)
    # fluent_np --> (1, 149, 768)
    if ((np.shape(fluent_np)[0] != 1) |(np.shape(fluent_np)[1] != 149) | (np.shape(fluent_np)[2] != 768)) :
        discarded_t += 1
...
print('Number of samples for train = ', n_samples_train)
print('Number of samples for validation = ', n_samples_valid)
print('Number of samples for test = ', n_samples_test)
    ...
    with torch.no_grad():
        for data in valid_loader:
            ...
            if(final_predicted is None):
                final_predicted = predicted_valid
                final_label = labels
            else:
                final_predicted = torch.cat((final_predicted,predicted_valid))
                final_label= torch.cat((final_label,labels))
        cf_matrix= confusion_matrix(final_label.cpu().detach().numpy(), 
                                    final_predicted.cpu().detach().numpy())
        print(cf_matrix)
...
with torch.no_grad():
   ...
   for features, labels in test_loader:
        ...
        if(final_predicted is None):
            ...
            final_predicted = torch.cat((final_predicted,predicted))
            final_label= torch.cat((final_label,labels))
...
cf_matrix= confusion_matrix(final_label.cpu().detach().numpy(), 
                            final_predicted.cpu().detach().numpy())
print(cf_matrix)
for i in range (0, final_predicted.shape[0]) :
    # F1 score for stutter
    if ((final_predicted[i] == 1) & (final_label[i] == 1)):
        TP +=1
    if (final_predicted[i] != final_label[i] and final_label[i]==0) :
        FP +=1
    if (final_predicted[i] != final_label[i] and final_label[i]==1) :
        FN +=1
    if (final_predicted[i] == final_label[i] and final_label[i]==0):
        TN +=1
