...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size).to(self.device)
...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True):
        ...
        if audioset_pretrain == False:
            ...
            self.mlp_head = nn.Sequential(nn.LayerNorm(self.original_embedding_dim), nn.Linear(self.original_embedding_dim, label_dim)).to(self.device)
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            if os.path.exists('../../pretrained_models/audioset_10_10_0.4593.pth') == False:
                ...
                wget.download(audioset_mdl_url, out='/LibriStutter_data/model/pretrained_models/audioset_10_10_0.4593.pth')
            sd = torch.load('/LibriStutter_data/model/pretrained_models/audioset_10_10_0.4593.pth', map_location=self.device)
    ...
    def forward(self, x):
        ...
        x = self.mlp_head(x.to("cpu")).to("cpu")
        return x
